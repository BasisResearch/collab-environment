...
gcs_browser: "Optional[GCSVideoBrowser]"
try:
...
async def index():
    ...
    with ui.column().classes("w-full p-4 gap-3"):
        ...
        with ui.row().classes("w-full items-center mb-2"):
            ...
            ui.space()
            ui.label("Real-time object detection and tracking").classes(
        ...
        with ui.card().classes("w-full shadow-md p-2"):
            with ui.row().classes("w-full gap-2 items-end"):
                ...
                if gcs_browser:
                    ...
                    video_select = ui.select(label="Video", options=[]).classes(
                        "flex-grow"
                    )
                ...
                ui.upload(
                    ...
                    "accept=video/mp4,video/quicktime,video/x-msvideo dense flat"
                ).props("label=Upload").style("width: 120px; height: 40px")

        # Row 2: Model/Params (left) | Controls + Preview (right)
        with ui.row().classes("w-full gap-3"):
            # LEFT: Model + Parameters (stacked)
            with (
                ui.column().classes("gap-3").style("flex: 0 0 280px; min-width: 280px")
            ):
                # Model card
                with ui.card().classes("w-full shadow-md p-3"):
                    ...
                    yolo_container.visible = (
                        prefs.get("model_source", "Roboflow") == "YOLO"
                    )
                    with yolo_container:
                        yolo_model_input = (
                            ui.input(
                            ...
                            .classes("w-full")
                            .tooltip("Enter any YOLO model name (will auto-download)")
                        )

                    # Roboflow model selection
                    rf_container = ui.column().classes("w-full mt-2 gap-2")
                    rf_container.visible = (
                    ...
                    with rf_container:
                        ...
                        async def list_rf_models():
                            ...
                            try:
                                ...
                                versions = model_manager.list_roboflow_project_models(
                                    project_id
                                )
                                if versions:
                                    options = {}
                                    ...
                                    for v in versions:
                                        ...
                                        _rf_versions_raw[v["version"]] = v.get(
                                            "raw", {}
                                        )
                        ...
                        def show_version_detail():
                            ...
                            with (
                                ui.dialog() as dlg,
                                ui.card().style("min-width: 500px; max-height: 80vh;"),
                            ):
                                ui.label(f"Version {ver} Details").classes(
                    ...
                    custom_container.visible = (
                        prefs.get("model_source", "Roboflow") == "Custom"
                    )
                    with custom_container:
                        ...
                        async def handle_model_upload(e):
                            ...
                            try:
                                ...
                                ui.notify(
                                    f"Model upload failed: {error}", type="negative"
                                )

                        ui.upload(
                            on_upload=handle_model_upload,
                            auto_upload=True,
                        ).props("accept=.pt dense flat").props(
                    ...
                    def enable_load_model_btn(e=None):
                        ...
                        if model_source.value == "YOLO" and yolo_model_input.value:
                            ...
                            model_source.value == "Roboflow" and rf_version_select.value
                        ):
                            load_model_btn.enable()
                        elif model_source.value == "Custom" and state.get(
                            "uploaded_model"
                        ):
                            load_model_btn.enable()
                    ...
                    if prefs.get("model_source") == "Roboflow" and prefs.get(
                        "rf_project_id"
                    ):
                        ...
                        async def _restore_rf_version():
                            ...
                            if saved_ver and saved_ver in (
                                rf_version_select.options or {}
                            ):
                ...
                with params_card:
                    ...
                    with ui.column().classes("w-full gap-1"):
                        ...
                        conf_slider.on(
                            "update:model-value",
                            lambda e: conf_label.set_text(f"Confidence: {e.args:.2f}"),
                        )
                        ...
                        for param_name, param_config in bytetrack_params_schema.items():
                            if param_config["type"] == "float":
                                ...
                                param_label = ui.label(
                                    f"{param_name.replace('_', ' ').title()}: {default_val:.2f}"
                                ).classes("text-xs")
                                ...
                                param_label = ui.label(
                                    f"{param_name.replace('_', ' ').title()}: {default_val}"
                                ).classes("text-xs")
                                param_label.tooltip(param_config["description"])

                                param_slider = ui.slider(
                                    min=min_val, max=max_val, step=1, value=default_val
                                ).classes("w-full")
                    ...
                    detection_only_checkbox.tooltip(
                        "Run detection without ByteTrack — shows raw detections per frame"
                    )

                    # Skip frames (for fast-forward, not a ByteTrack param)
                    with ui.row().classes("w-full items-center gap-2 mt-2"):
                        skip_frames_label = ui.label("Skip: every frame").classes(
                            "text-xs"
                        )
                        skip_frames_slider = ui.slider(
                            min=1, max=30, step=1, value=1
                        ).style("width: 100px")
                        skip_frames_slider.tooltip(
                            "Process every Nth frame (1 = all frames)"
                        )
                        skip_frames_slider.on(
                            "update:model-value",
                            lambda e: skip_frames_label.set_text(
                                "Skip: every frame"
                                if int(e.args) == 1
                                else f"Skip: every {int(e.args)} frames"
                            ),
                        )
                    ...
                    with ui.row().classes("w-full items-center gap-2 mt-1"):
                        ...
                        display_update_slider = ui.slider(
                            min=1, max=30, step=1, value=1
                        ).style("width: 100px")
                        display_update_slider.tooltip(
                            "Update display every Nth frame (1 = every frame, higher = skip display frames)"
                        )
                        display_update_slider.on(
                            "update:model-value",
                            lambda e: display_update_label.set_text(
                                "Display: every frame"
                                if int(e.args) == 1
                                else f"Display: every {int(e.args)} frames"
                            ),
                        )
            ...
            with ui.column().classes("flex-grow gap-3"):
                ...
                with ui.card().classes("w-full shadow-md p-3"):
                    ...
                    with ui.row().classes("w-full items-center gap-2 mb-2"):
                        load_video_btn = ui.button("Load Video").props(
                            "color=primary icon=video_file"
                        )
                        load_video_btn.disable()  # Enabled when video selected

                        load_model_btn = ui.button("Load Model").props(
                            "color=primary icon=model_training"
                        )
                        ...
                        with ui.column().classes("flex-grow gap-1"):
                            status_label = ui.label("Select video and model").classes(
                                "text-xs"
                            )

                    # Row 2: Playback controls
                    with ui.row().classes("w-full items-center gap-2"):
                        start_btn = ui.button("Start Tracking").props(
                    ...
                    with ui.column().classes("w-full gap-1 mt-2"):
                        time_label = ui.label("Frame: 0 / 0").classes(
                            "text-xs text-gray-600"
                        )
                        time_slider = ui.slider(min=0, max=100, value=0).classes(
                            "w-full"
                        )
                        time_slider.disable()
                        ...
                        async def seek_to_frame(e):
                            ...
                            target_frame = int(e.args)
                            if state.get("processing") and state.get(
                                "skip_frames_event"
                            ):
                                current_frame = state.get("current_frame", 0)
                                if target_frame != current_frame:
                                    state["skip_frames_event"]["skip_amount"] = (
                                        target_frame - current_frame
                                    )
                                    ui.notify(
                                        f"Seeking to frame {target_frame}...",
                                        type="info",
                                    )
                ...
                with ui.card().classes("w-full shadow-md p-3"):
                    ...
                    video_container = (
                        ui.element("div")
                        .classes("border-2 border-gray-200 rounded bg-gray-50")
                        .style("max-width: 100%; resize: horizontal; overflow: hidden;")
                    )
                    with video_container:
                        video_display = ui.interactive_image("").style("width: 100%;")
        ...
        try:
            ...
            if await asyncio.to_thread(needs_conversion, local_video):
                ...
                if is_h264:
                    ...
                    await asyncio.to_thread(
                        convert_to_h264, local_video, converted_video, remux_only=True
                    )
                else:
                    status_label.text = "Converting to H.264..."
                    await asyncio.to_thread(
                        convert_to_h264, local_video, converted_video
                    )
            ...
            with client:
                ...
                if ret:
                    _, buf = cv2.imencode(
                        ".jpg", first_frame, [cv2.IMWRITE_JPEG_QUALITY, 80]
                    )
                    b64 = base64.b64encode(buf).decode()
            ...
            with client:
                status_label.text = "Error loading video"
        ...
        try:
            ...
            if model_source.value == "YOLO":
                ...
                if not project_id or not version:
                    raise ValueError(
                        "Please select a Roboflow model (project ID and version)"
                    )
            ...
            with client:
                ...
                tracker_type = (
                    "YOLO Native" if isinstance(model, YOLO) else "Supervision"
                )
            ...
            with client:
                status_label.text = "Error loading model"
                ui.notify(f"Error: {str(e)}", type="negative")
        finally:
            with client:
                load_model_btn.enable()
    ...
    async def _run_processing(save_csv: bool, force_start_frame: Optional[int] = None):
        ...
        try:
            ...
            debug_params_label.text = "  ".join(
                f"{k}={v}" for k, v in active_params.items()
            )
