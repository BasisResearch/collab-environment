...
def node_feature_vel_plus_pos_plus(past_p,past_v,past_a,
            species_idx,species_dim,past_time = 3):
    """
    Only has the agent velocities and position as feature.
    """
...
def run_gnn(model,
    ...
    if debug_result is None:
        ...
        debug_result["predicted"] = []   # where we keep the predicted result of frame t + 1 from information 1:t
        debug_result["actual_acc"] = []  # where we keep the actual acceleration at frame t.
        debug_result["predicted_acc"] = [] # where we keep the predicted acceleration at frame t.
        debug_result["loss"] = []        # where we keep loss for prediction of acceleration at frame t.
    ...
    pos, vel, acc, v_function = handle_discrete_data(position, model.input_differentiation)
...
def get_adjcency_from_debug(debug_result, input_data_loader, visual_range, epoch_list = None):
    """
    ...
    for epoch in epoch_list:
        ...
        for batch_ind in W_across_batch.keys():
            ...
            for f in frame: #over frames
                edge_index_input = build_edge_index(position[:,f,:,:], visual_range = visual_range)
                W_input_frame = normalize_by_col(N, edge_index_input, return_matrix = True)

                (edge_index_output, edge_weight_output) = W_across_batch[batch_ind]['W'][f]
                W_output = return_adjacency_matrix(N, edge_index_output, edge_weight_output)
