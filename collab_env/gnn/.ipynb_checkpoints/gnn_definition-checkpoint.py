...
class GNN(torch.nn.Module):
    def __init__(self, model_name, in_node_dim,
        ...
        """
        self.gatn = GATConv(in_node_dim, hidden_dim, edge_dim=1, 
                              heads = heads, add_self_loops = False)
        #self.gatn = GATv2Conv(in_node_dim, hidden_dim, edge_dim=1,
        #                      heads = heads, add_self_loops = False)
        self.gcn2 = GCNConv(hidden_dim * heads, hidden_dim,
                            add_self_loops = False)
    ...
    def forward(self, x, edge_index, edge_weight):
        ...
        h_tmp, W = self.gatn(x, edge_index, edge_attr = edge_weight,
...
class Lazy(torch.nn.Module):
    ...
    def __init__(self, model_name, in_node_dim = 3,
                 heads = 1, hidden_dim=30,
                 input_differentiation = "finite", # or "spline"
                 prediction_integration = "Euler"): # or "Leapfrog")
        super().__init__()
        self.name = model_name
