...
class BoidsWorldAgent:
    ...
    def __init__(
        ...
        self.q_values = [defaultdict(lambda: np.zeros(env.action_space.n)) for _ in range(num_agents)]
    ...
    def simpleBoidsAction(self, obs):
        ...
        for i in range(self.num_agents):
            ...
            '''
            if (not self.walking) and (obs['mesh_distance'][i] < self.min_ground_separation):
                ...
                if num_close > 0:
                    print('simpleBoidsAction(): num close to ' + str(i) + ' is ' + str(num_close))
                ...
                if not self.walking and obs['mesh_distance'][i] < self.min_ground_separation:
                    ...
                    ground_force = ground_diff_vector / (np.linalg.norm(ground_diff_vector) ** 2)
                    print('SBA(): ground_diff_vector ', ground_diff_vector)
                    print('SBA(): ground force ' + str(ground_force))
                    # sum_separation_vector += diff_vector/(dist**2)

                total_force += (self.alignment_weight * align_force + self.cohesion_weight * cohesion_force
                                + self.target_weight * target_force + self.separation_weight * separation_force
