...
import collab_env.sim.gymnasium_env # this is a required import even though IDE shows it as unused.
...
if __name__ == '__main__':
    ...
    learning_rate = 0.01  # How fast to learn (higher = faster but less stable) -- not used
    n_episodes = 1  # Number of boids runs.
    start_epsilon = 0.0 # Start with 100% random actions -- not used
    epsilon_decay = start_epsilon / (n_episodes / 2)  # Reduce exploration over time == not sued
    ...
    env = gym.make("gymnasium_env/BoidsWorldSimple-v0", render_mode='human', num_agents=NUM_AGENTS, walking=WALKING)
