...
class BoidsWorldSimpleEnv(gym.Env):
        ...
        self.observation_space = spaces.Dict(
            {
                "agent_loc": spaces.Tuple(
                    [spaces.Box(low=-torch.inf, high=torch.inf, shape=(3,), dtype=np.float64) for _ in
                     range(num_agents)]),
                ...
                "target_loc": spaces.Box(low=-torch.inf, high=torch.inf, shape=(3,), dtype=np.float64),

                "norms": spaces.Tuple([spaces.Box(low=-torch.inf, high=torch.inf, shape=(1,), dtype=np.float64) for _ in
                                       range(self.num_agents)]),

                "mesh_distance": spaces.Tuple(
                    [spaces.Box(low=-torch.inf, high=torch.inf, shape=(1,), dtype=np.float64) for _ in
                     range(self.num_agents)]),
    ...
    def _get_obs(self):
        ...
        return {"agent_loc": tuple(self._agent_location),
    ...
    def compute_distances(self):
        ...
        norms = tuple(np.array([np.linalg.norm(a - self._target_location) for a in self._agent_location], dtype='f'))
        return norms
    ...
    def reset(self, seed=None, options=None):
        ...
        '''
        if self.walking:
            self._agent_location = [np.array([np.random.uniform(low=0.1 * self.box_size, high=0.9 * self.box_size),
                                              0.0,
                                              np.random.uniform(low=0.1 * self.box_size, high=0.9 * self.box_size)])
                                    for _ in range(self.num_agents)]

        else:
            self._agent_location = [self.np_random.uniform(low=0.1 * self.box_size,
                                                           high=0.9 * self.box_size,
                                                           size=3) for _ in range(self.num_agents)]
        ...
        '''
        self._agent_velocity = [self.np_random.normal(0, 0.2, size=3) for _ in range(self.num_agents)]
        '''
        TOC -- 072125
        '''
        ...
        self._target_location = np.array([self.box_size / 2, self.box_size / 2, self.box_size / 2])
        self._ground_target_location = np.array([self.box_size / 2.0, 0, self.box_size / 2])
        ...
        while np.linalg.norm(self._target_location - np.array([0., 0., 0.])) < 0.000001:
            self._target_velocity = self.np_random.normal(0, 0.1, size=3)
    ...
    def step(self, action):
        ...
        for i in range(self.num_agents):
            for coordinate in range(3):
                if ((self._agent_location[i][coordinate] + self._agent_velocity[i][coordinate] < 0)
                        or (self._agent_location[i][coordinate] + self._agent_velocity[i][coordinate] > self.box_size)):
                    self._agent_velocity[i][coordinate] *= np.random.normal(-0.8, 0.1, size=1)
        ...
        '''
        if self.walking:
            self._agent_velocity = self.convert_to_velocity_along_mesh(self._agent_location, self._agent_velocity)
            # distances, closest_points = self.compute_distance_and_closest_points(self.mesh_scene,
            #                                                                     self._agent_location + self._agent_velocity)
            # new_locations = closest_points.numpy()
            # self._agent_velocity = new_locations - self._agent_location

        self._ground_target_velocity = self.convert_to_velocity_along_mesh(np.array([self._ground_target_location]),
                                                                           np.array([self._ground_target_velocity]))[0]

        # update the agent's position based on its velocity
        self._agent_location = np.array(self._agent_location) + np.array(self._agent_velocity)
        ...
        # update the targets
        self._ground_target_location = self._ground_target_location + self._ground_target_velocity
        ...
        '''
        terminated = len(distance[distance < HIT_DISTANCE]) > 0  # np.array_equal(self._agent_location, self._target_location)
    ...
    def load_rotate_scene(self, filename, position=np.array([0.0, 0.0, 0.0]), angles=(-np.pi / 2, 0, 0)):
    ...
    def _render_frame(self):
        ...
        if self.render_mode == 'human':
            if self.vis is None:
                ...
                filename = expand_path('meshes/Open3dTSDFfusion_mesh.ply', get_project_root())
                self.mesh_scene = self.load_rotate_scene(filename,
                                                         # self.mesh_scene = self.load_rotate_scene("mesh/query-tree.ply",
                                                         position=np.array(
                                                             [self.box_size / 2.0, -self.box_size / 2.0, 8.0]),
                                                         angles=(-np.pi / 2, 0, 0))
                ...
                for i in range(self.num_agents):
                    ...
                    self.mesh_arrow_agent[i] = open3d.geometry.TriangleMesh.create_arrow(cone_height=0.8,
                                                                                         cone_radius=0.4,
                                                                                         cylinder_radius=0.4,
                                                                                         cylinder_height=1.0)
                    ...
                    self.mesh_arrow_agent[i].scale(scale=2, center=self._agent_location[i])
                    self.vis.update_geometry(self.mesh_arrow_agent[i])
                ...
                self.mesh_sphere_target = open3d.geometry.TriangleMesh.create_sphere(radius=1.5)
                self.mesh_sphere_target.compute_vertex_normals()
                self.mesh_sphere_target.paint_uniform_color([0.1, 0.6, 0.1])

                self.mesh_sphere_ground_target = open3d.geometry.TriangleMesh.create_sphere(radius=0.4)
