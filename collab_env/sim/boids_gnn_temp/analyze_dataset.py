...
def analyze_dataset(dataset_path, save_dir=None, device="cpu", scene_size=480.0):
    ...
    for idx, (positions_traj, _) in enumerate(dataset):
        ...
        velocities_np = np.concatenate(
            [velocities_np, velocities_np[-1:]], axis=0
        )  # [T, N, 2]
    ...
    fig.suptitle(
        f"Dataset EDA: {os.path.basename(dataset_path)}", fontsize=16, fontweight="bold"
    )
    ...
    ax = axes[0, 1]
    ax.hist2d(
        velocities_all[:, 0], velocities_all[:, 1], bins=50, cmap="viridis", cmin=1
    )
    ...
    ax.grid(True, alpha=0.3)
    ax.axvline(
        speeds.mean(), color="red", linestyle="--", label=f"Mean: {speeds.mean():.3f}"
    )
    ax.legend()
    ...
    ax.axvline(
        ...
        color="red",
        linestyle="--",
        label=f"Mean: {distances_all.mean():.3f}",
    )
    ax.legend()
    ...
    ax.axvline(
        ...
        color="red",
        linestyle="--",
        label=f"Mean: {delta_vel_mag.mean():.3f}",
    )
    ax.legend()
    ...
    sample_indices = np.random.choice(
        len(delta_vel_all), size=min(50000, len(delta_vel_all)), replace=False
    )
    ax.hist2d(
        delta_vel_all[sample_indices, 0],
        delta_vel_all[sample_indices, 1],
        bins=50,
        cmap="plasma",
        cmin=1,
    )
    ...
    if os.path.exists(config_path):
            ...
            if species_key and species_key != "scene_size":
                ...
                if "min_distance" in species_config:
                    min_distance_normalized = (
                        species_config["min_distance"] / scene_size
                    )

                if save_dir:
                    true_plot_path = os.path.join(
                        save_dir, "true_boid_force_fields.png"
                    )
                    plot_force_decomposition(
                        true_forces,
                ...
                if save_dir:
                    symmetric_plot_path = os.path.join(
                        save_dir, "true_boid_symmetric_decomposition.png"
                    )
                    plot_symmetric_force_decomposition(
                        pos_forces_true,
                        vel_forces_true,
                    ...
                    logger.info(
                        f"Saved symmetric decomposition plot to {symmetric_plot_path}"
                    )
                else:
                    plot_symmetric_force_decomposition(
                        pos_forces_true,
        ...
        logger.info(
            f"Config file not found at {config_path}, skipping force field plots"
        )
