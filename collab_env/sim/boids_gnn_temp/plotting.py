...
def plot_true_boid_rules_2d(config=None, scene_size=480.0, save_path=None):
    ...
    skip = 5
    quiver1 = ax1.quiver(
        X[::skip, ::skip] * scene_size,
        Y[::skip, ::skip] * scene_size,
        sep_force_x[::skip, ::skip],
    ...
    im2 = ax2.contourf(
        X * scene_size, Y * scene_size, sep_magnitude, levels=20, cmap="Reds"
    )
    circle2 = plt.Circle(
        (0, 0),
        config["min_distance"],
    ...
    ax3.set_title(
        "Cohesion Force\n(Attraction within visual_range)",
        fontsize=11,
        fontweight="bold",
    )
    ...
    im4 = ax4.contourf(
        X * scene_size, Y * scene_size, coh_magnitude, levels=20, cmap="Blues_r"
    )
    circle4 = plt.Circle(
        (0, 0),
        config["visual_range"],
    ...
    im6 = ax6.contourf(
        X * scene_size, Y * scene_size, total_magnitude, levels=20, cmap="viridis"
    )
    circle6a = plt.Circle(
        (0, 0),
        config["min_distance"],
    ...
    plt.suptitle(
        "True 2D Boid Rules (Position-Dependent Forces)",
        fontsize=14,
        fontweight="bold",
        y=0.995,
    )
...
def plot_force_decomposition(
    ...
    def plot_force_field(ax, force_dict, title):
        ...
        if visual_range is not None:
            circle = plt.Circle(
                ...
                alpha=0.6,
                zorder=5,
                label=f"Visual range ({visual_range:.3f})",
            )
            ax.add_patch(circle)
        ...
        if min_distance is not None:
            min_circle = plt.Circle(
                ...
                alpha=0.6,
                zorder=5,
                label=f"Min distance ({min_distance:.3f})",
            )
    ...
    ax1 = plt.subplot(2, 3, 1)
    q1 = plot_force_field(
        ax1, forces_dict["away_total"], "I) Total Force\n(vel_j away from i)"
    )
    ...
    ax3 = plt.subplot(2, 3, 3)
    q3 = plot_force_field(
        ax3, forces_dict["away_vel"], "III) Velocity Residual\n(I - II)"
    )
    ...
    ax4 = plt.subplot(2, 3, 4)
    q4 = plot_force_field(
        ax4, forces_dict["towards_total"], "I) Total Force\n(vel_j towards i)"
    )
    ...
    ax6 = plt.subplot(2, 3, 6)
    q6 = plot_force_field(
        ax6, forces_dict["towards_vel"], "III) Velocity Residual\n(I - II)"
    )
    plt.colorbar(q6, ax=ax6, label="Force mag")

    plt.suptitle(
        f"{title_prefix} Force on j due to i at origin (arrows show force on j at that position)",
        fontsize=14,
        fontweight="bold",
        y=0.995,
    )
...
def evaluate_true_boid_forces(config, grid_size=60, max_dist=50.0, scene_size=480.0):
    ...
    def compute_boid_forces(vel_j_array):
        ...
        for idx in range(n_points):
            ...
            boid_i = {"x": 0.0, "y": 0.0, "dx": 0.0, "dy": 0.0, "species": "A"}
            ...
            boid_i_zero = {"x": 0.0, "y": 0.0, "dx": 0.0, "dy": 0.0, "species": "A"}
...
def evaluate_true_boid_velocity_forces(
    config, grid_size=60, max_vel=0.05, scene_size=480.0
):
    """
    ...
    for idx in range(n_points):
        ...
        boid_i = {"x": 0.5, "y": 0.5, "dx": 0.0, "dy": 0.0, "species": "A"}
...
def plot_symmetric_force_decomposition(
    position_forces,
    velocity_forces,
    save_path=None,
    title_prefix="Learned",
    ...
    im = ax.pcolormesh(
        X, Y, position_forces["away_pos"]["mag"], cmap="viridis", shading="auto"
    )
    ...
    # Plot force magnitude as heatmap
    im = ax.pcolormesh(
        VX, VY, velocity_forces["force_mag"], cmap="plasma", shading="auto"
    )
    plt.colorbar(im, ax=ax, label="Force Magnitude")
