...
class QueryBackend:
    ...
    def __init__(
        self, config: Optional[DBConfig] = None, backend: Optional[str] = None
    ):
        """
    ...
    def _execute_query(self, query_name: str, **params) -> pd.DataFrame:
        ...
        **params
        ...
        try:
            ...
            elapsed_time = time.time() - start_time
            logger.error(
                f"Query '{query_name}' execution failed after {elapsed_time:.3f}s: {e}"
            )
            raise
    ...
    try:
        ...
        if len(sessions) > 0:
            ...
            if len(episodes) > 0:
                ...
                print(
                    f"  Density range: {heatmap['density'].min():.0f} - {heatmap['density'].max():.0f}"
                )

                # Display heatmap data
                print("\n  Heatmap sample (top 10 densest bins):")
                top_bins = heatmap.nlargest(10, "density")[
