    ...
    def _open_mesh_3d_viewer(self, _):
        ...
        try:
            ...
            if self._current_camera_params_file:
                ...
                camera_params_cache_path = self.file_manager.get_cache_path(
                    bucket, camera_params_path
                )
                logger.info("🔍 Camera params path construction:")
                ...
                if not Path(camera_params_cache_path).exists():
                    ...
                    try:
                        logger.info(
                            f"📷 Downloading camera params: {bucket}/{camera_params_path}"
                        )
                        params_content = self.rclone_client.read_file(
                            bucket, camera_params_path
                        )
                        logger.info(
                            f"📏 Downloaded {len(params_content)} bytes of camera params"
                        )
                        Path(camera_params_cache_path).write_bytes(params_content)
                        logger.info(
                            f"✅ Camera params cached to: {camera_params_cache_path}"
                        )
                    except Exception as e:
                        logger.error(
                            f"❌ Failed to download camera params {bucket}/{camera_params_path}: {e}"
                        )
                        camera_params_cache_path = None
                    finally:
                        self._hide_loading()
                    ...
                    logger.info(
                        f"📋 Camera params already cached: {camera_params_cache_path}"
                    )
            ...
            try:
                ...
                add_mesh_data = {
                    ...
                    "camera_params_path": str(camera_params_cache_path)
                    if camera_params_cache_path
                    else None,
        ...
        for preferred_camera in priority_order:
            for csv_file in csv_3d_files:
                if csv_file["camera"] == preferred_camera:
                    logger.info(
                        f"Selected 3D CSV: {csv_file['display_name']} (priority: {preferred_camera})"
                    )
                    return csv_file

        # Fallback to first file if no matches
        logger.info(
            f"No priority match, using first file: {csv_3d_files[0]['display_name']}"
        )
        return csv_3d_files[0]
    ...
    def _stop_all_bbox_viewers(self, _=None):
        ...
        try:
            ...
            if health_response.status_code == 200:
                ...
                logger.info(
                    f"Server currently has {current_videos} videos and {current_meshes} meshes"
                )
            ...
            if clear_response.status_code == 200:
                ...
                total_cleared = data.get(
                    "total_cleared", cleared_videos + cleared_meshes
                )

                logger.info(
                    f"Cleared {cleared_videos} videos and {cleared_meshes} meshes from server (total: {total_cleared})"
                )
                ...
                if total_cleared > 0:
                    ...
                    self.status_pane.object = (
                        "<p style='color:blue'>ℹ️ Server was already empty</p>"
                    )
                api_success = True
            else:
                logger.warning(
                    f"Clear API failed with status {clear_response.status_code}"
                )
