...
class ExtendedPropertiesViewerWidget(BaseAnalysisWidget):
    ...
    widget_description = (
        "Time series and histogram visualization of extended properties"
    )
    widget_category = "visualization"

    # Property selection (for time series and histograms)
    selected_properties = param.ListSelector(
        default=[], objects=[], doc="Selected properties to display"
    )

    # Quantile band parameters
    lower_quantile = param.Number(
        default=0.10,
        bounds=(0.01, 0.49),
        doc="Lower quantile for uncertainty band (default: 10th percentile)",
    )
    upper_quantile = param.Number(
        default=0.90,
        bounds=(0.51, 0.99),
        doc="Upper quantile for uncertainty band (default: 90th percentile)",
    )

    # Raw datapoints display options
    show_raw_lines = param.Boolean(
        default=False, doc="Show individual agent trajectories as lines"
    )
    raw_line_opacity = param.Number(
        default=0.3, bounds=(0.05, 1.0), doc="Opacity of individual agent lines"
    )
    raw_marker_size = param.Integer(
        default=4, bounds=(1, 12), doc="Size of markers on individual agent lines"
    )
    max_agents_to_plot = param.Integer(
        default=20, bounds=(1, 100), doc="Maximum number of agent lines to display"
    )

    # Normalization option
    normalize = param.Boolean(
        default=False, doc="Z-score normalize all properties to display on same scale"
    )
    ...
    plot_type = param.Selector(
        default="Histogram",
        objects=["Histogram", "Violin Plot (separate)", "Violin Plot (merged)"],
    ...
    def create_custom_controls(self) -> Optional[pn.Column]:
        ...
        self.load_names_btn = pn.widgets.Button(
            name="1. Load Property Names", button_type="primary", width=200
        )
        ...
        property_selection_section = pn.Column(
            ...
            pn.Row(
                self.load_data_btn,
                pn.pane.Markdown(
                    "_Load only selected properties_",
                    styles={"color": "#666", "font-style": "italic"},
                ),
            ),
            pn.pane.Markdown(
        ...
        lower_quantile_slider = pn.widgets.FloatSlider.from_param(
            self.param.lower_quantile, name="Lower Quantile", width=150, step=0.05
        )

        upper_quantile_slider = pn.widgets.FloatSlider.from_param(
            self.param.upper_quantile, name="Upper Quantile", width=150, step=0.05
        )

        # Normalize checkbox
        normalize_checkbox = pn.widgets.Checkbox.from_param(
            self.param.normalize, name="Z-Score Normalize"
        )

        # Raw datapoints controls
        show_raw_checkbox = pn.widgets.Checkbox.from_param(
            self.param.show_raw_lines, name="Show Agent Lines"
        )

        opacity_slider = pn.widgets.FloatSlider.from_param(
            self.param.raw_line_opacity, name="Opacity", width=120
        )

        marker_size_slider = pn.widgets.IntSlider.from_param(
            self.param.raw_marker_size, name="Marker Size", width=120
        )

        max_agents_slider = pn.widgets.IntSlider.from_param(
            self.param.max_agents_to_plot, name="Max Agents", width=120
        )

        # Distribution plot controls
        bin_count_slider = pn.widgets.IntSlider.from_param(
            self.param.bin_count, name="Histogram Bins", width=150
        )

        plot_type_selector = pn.widgets.Select.from_param(
            self.param.plot_type, name="Plot Type", width=150
        )
        ...
        viz_options = pn.Column(
            ...
            pn.Row(bin_count_slider, plot_type_selector, sizing_mode="stretch_width"),
        )
    ...
    def _on_load_names_click(self, event):
        ...
        try:
            ...
            self.available_props_df = self.query_with_context(
            ...
            previous_selection = (
                list(self.selected_properties) if self.selected_properties else []
            )
            preserved_selection = [
                prop for prop in previous_selection if prop in prop_list
            ]
    ...
    def _on_load_data_click(self, event):
        ...
        try:
            self.context.report_loading(
                f"Loading {len(self.selected_properties)} selected properties..."
            )
            ...
            self.context.report_success(
                f"Loaded {len(self.selected_properties)} properties successfully"
            )
    ...
    def load_data(self, property_ids: Optional[List[str]] = None) -> None:
        ...
        if self.context.scope.scope_type not in [ScopeType.EPISODE, ScopeType.SESSION]:
            raise ValueError(
                "Extended Properties Viewer only supports episode and session scopes"
            )
        ...
        logger.info(
            f"Loading {len(property_ids)} properties for {scope_label}: {property_ids}"
        )
        ...
        if self.context.scope.scope_type == ScopeType.EPISODE:
            ...
            logger.info(
                f"Loading property time series (quantiles: {self.lower_quantile:.0%} - {self.upper_quantile:.0%})..."
            )
            ...
            self.display_pane.objects = [
                ...
                pn.pane.HoloViews(
                    timeseries_plot, sizing_mode="stretch_width", height=400
                ),
            ...
            logger.info(
                f"Loading property distributions (session-level aggregation{time_filter_msg})..."
            )
            ...
            self.display_pane.objects = [
                ...
                pn.pane.Markdown(
                    "_Time series plots are not available for session-level analysis_"
                ),
                histogram_pane,
            ]

        logger.info(
            f"Extended Properties Viewer loaded successfully ({scope_label} scope, {len(property_ids)} properties)"
        )
    ...
    def _create_timeseries_plot(self):
        ...
        if self.properties_ts_df is None or len(self.properties_ts_df) == 0:
            ...
            return hv.Curve([]).opts(
                width=800, height=400, title="Time Series (no properties available)"
            )
        ...
        df = self.properties_ts_df[
            self.properties_ts_df["property_id"].isin(self.selected_properties)
        ]
        ...
        for i, prop_id in enumerate(self.selected_properties):
            ...
            if "q_lower" in prop_df.columns and "q_upper" in prop_df.columns:
                ...
                if len(quantile_df) > 0:
                    ...
                    if self.normalize:
                        quantile_df["median_value"] = self._normalize_values(
                            quantile_df["median_value"].values, mean, std
                        )
                        quantile_df["q_lower"] = self._normalize_values(
                            quantile_df["q_lower"].values, mean, std
                        )
                        quantile_df["q_upper"] = self._normalize_values(
                            quantile_df["q_upper"].values, mean, std
                        )
                    ...
                    spread = hv.Spread(
                        (
                            quantile_df["time_window"],
                            quantile_df["median_value"],
                            neg_err,
                            pos_err,
                        ),
                        kdims="Time Window",
        ...
        if not has_data:
            ...
            return hv.Curve([]).opts(
                width=800, height=400, title="Time Series (no valid data)"
            )
        ...
        def layout_hook(plot, element):
            ...
            fig["layout"]["title"] = dict(text=title_text, font=dict(size=9))

        # Create overlay and apply options
        scope_str = (
            str(self.context.scope)
    ...
    def _create_histogram_layout(self):
        ...
        # Filter to selected properties
        df = self.properties_dist_df[
            self.properties_dist_df["property_id"].isin(self.selected_properties)
        ]
        ...
        if not hasattr(self, "property_color_map") or set(
            self.property_color_map.keys()
        ) != set(self.selected_properties):
            self.property_color_map = {
                prop_id: self.colors[i % len(self.colors)]
                for i, prop_id in enumerate(self.selected_properties)
            }
    ...
    def _create_histograms(self, df: pd.DataFrame):
        ...
        if histogram_plots:
            ...
            return hv.Curve([]).opts(
                width=800, height=300, title="Histograms (no valid data)"
            )
    ...
    def _create_separate_violins(self, df: pd.DataFrame):
        ...
        for prop_id in self.selected_properties:
            ...
            try:
                ...
                violin_df = pd.DataFrame(
                    {"Property": [prop_id] * len(values), "Value": values}
                )
        ...
        if violin_plots:
            ...
            return hv.Curve([]).opts(
                width=800, height=300, title="Violin plots (no valid data)"
            )
    ...
    def _create_merged_violin(self, df: pd.DataFrame):
        ...
        if not all_values:
            return hv.Curve([]).opts(
                width=800, height=300, title="Merged violin plot (no valid data)"
            )
        ...
        try:
            ...
            def color_hook(plot, element):
                ...
                try:
                    ...
                    if ax is None:
                        logger.warning(
                            "Could not access matplotlib axis for color customization"
                        )
                        return

                    # Get violin parts (PolyCollection objects)
                    if hasattr(ax, "collections") and len(ax.collections) > 0:
                        for i, (collection, prop_id) in enumerate(
                            zip(ax.collections, self.selected_properties)
                        ):
    ...
    def _create_agent_lines(self, prop_id: str, base_color: str) -> List[hv.Curve]:
        ...
        prop_raw = self.properties_raw_df[
            self.properties_raw_df["property_id"] == prop_id
        ].copy()
        ...
        if self.normalize:
            mean, std = self._get_normalization_stats(prop_id)
            prop_raw["value_float"] = self._normalize_values(
                prop_raw["value_float"].values, mean, std
            )
        ...
        if len(agents) > self.max_agents_to_plot:
            logger.info(
                f"Limiting agent lines from {len(agents)} to {self.max_agents_to_plot} agents"
            )
        ...
        for agent_id in agents:
            agent_data = prop_raw[prop_raw["agent_id"] == agent_id].sort_values(
                "time_index"
            )
            ...
            scatter = hv.Scatter(
                ...
                kdims="Time Window",
                vdims="Value",
            ).opts(color=rgba_color, size=self.raw_marker_size, show_legend=False)
    ...
    def _on_properties_change(self):
        ...
        if (
            self.properties_ts_df is not None or self.properties_dist_df is not None
        ) and len(self.display_pane.objects) > 1:
            self._update_plots()
    ...
    def _on_show_raw_lines_change(self):
        ...
        if self.show_raw_lines and self.properties_raw_df is None:
            ...
            property_ids = (
                list(self.selected_properties) if self.selected_properties else None
            )
            self.properties_raw_df = self.query_with_context(
                "get_extended_properties_raw", property_ids=property_ids
            )
            logger.info(f"Loaded {len(self.properties_raw_df)} raw observations")
    ...
    @param.depends(
        "raw_line_opacity", "raw_marker_size", "max_agents_to_plot", watch=True
    )
    def _on_raw_params_change(self):
        """Handle changes to raw line parameters."""
        if (
            self.show_raw_lines
            and self.properties_raw_df is not None
            and len(self.display_pane.objects) > 1
        ):
            self._update_plots()
    ...
    def _on_quantile_change(self):
        ...
        logger.info(
            f"Reloading time series with new quantiles: {self.lower_quantile:.0%} - {self.upper_quantile:.0%}"
        )
        ...
        if isinstance(histogram_layout, pn.GridBox):
            ...
            if self.plot_type == "Violin Plot (merged)":
                return pn.pane.HoloViews(
                    histogram_layout, sizing_mode="stretch_width", height=500
                )
            else:
                return pn.pane.HoloViews(
                    histogram_layout, sizing_mode="stretch_width", min_height=300
                )
