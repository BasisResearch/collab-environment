...
class BasicDataViewerWidget(BaseAnalysisWidget):
    ...
    # Playback controls
    current_time = param.Integer(
        default=0, bounds=(0, None), doc="Current playback time index"
    )
    playback_speed = param.Number(
        default=1.0, bounds=(0.1, 10.0), doc="Default playback speed for modal viewer"
    )
    trail_length = param.Integer(
        default=50, bounds=(0, 500), doc="Number of frames to show in trail"
    )
    ...
    def __init__(self, **params):
        ...
        self.agent_color_map: Optional[dict[Any, str]] = None
    ...
    def create_display_pane(self) -> pn.pane.PaneBase:
        ...
        return pn.Row(
            self.animation_pane, self.heatmap_pane, sizing_mode="stretch_both"
        )
    ...
    def load_data(self) -> None:
        ...
        if scope_type == ScopeType.EPISODE:
            ...
            self.agent_color_map = {
                agent_id: Category20_20[i % len(Category20_20)]
            ...
            logger.info(
                "Basic Data Viewer loaded successfully (session scope - heatmap only)"
            )
    ...
    def get_animation_data_json(self):
        ...
        agent_colors_serializable = {
            int(agent_id): color for agent_id, color in self.agent_color_map.items()
        }
        ...
        return {
            ...
            "bounds": {
                ...
                "z_range": [float(self.z_range[0]), float(self.z_range[1])]
                if self.z_range
                else None,
            },
    ...
    def _update_animation_panel(self):
        ...
        start_time = max(
            self.current_time - self.trail_length, self.tracks_df["time_index"].min()
        )
        ...
        if self.dimensionality == Dimensionality.THREE_D:
            ...
            if len(current_df) > 0:
                ...
                current_df["agent_color"] = current_df["agent_id"].map(
                    self.agent_color_map
                )
            ...
            if self.trail_length > 0 and len(window_df) > 0:
                ...
                for agent_id in window_df["agent_id"].unique():
                    agent_trail = window_df[
                        window_df["agent_id"] == agent_id
                    ...
                    if len(agent_trail) > 1:
                        ...
                        if len(agent_trail) > 1:
                            ...
                            path = hv.Path3D(agent_trail, kdims=["x", "y", "z"]).opts(
                                color=agent_color, line_width=2
                            )
                            paths.append(path)
            ...
            if self.trail_length > 0 and len(window_df) > 0:
                ...
                for agent_id in window_df["agent_id"].unique():
                    ...
                    if len(agent_trail) > 1:
                        ...
                        if len(agent_trail) > 1:
                            agent_color = self.agent_color_map.get(agent_id, "gray")
                            path = hv.Path(agent_trail, kdims=["x", "y"]).opts(
                                color=agent_color, line_width=2
                            )
                            paths.append(path)
    ...
    def _update_heatmap_panel(self):
        ...
        if self.dimensionality == Dimensionality.ONE_D:
            ...
            viz = hv.Scatter3D(
                df, kdims=["x_center", "y_center", "z_center"], vdims="density"
            ).opts(
                color="density",
            ...
            if not x_range_ok or not y_range_ok:
                ...
                viz = hv.Scatter(
                    df, kdims=["x_center", "y_center"], vdims="density"
                ).opts(
                    color="density",
                    cmap=self.color_scale,
                    size=10,
    ...
    def _show_animation_modal(self, animation_data):
        ...
        if num_points > 10000:
            logger.warning(
                f"Large dataset: {num_points} track points - animation may be slow"
            )
