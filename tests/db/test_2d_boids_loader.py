...
class TestBoids2DLoader:
    ...
    def test_load_2d_boids_dataset(
        self, backend_config: DBConfig, sample_2d_boids_data: Path
    ):
        ...
        db.execute(
            "DELETE FROM episodes WHERE session_id LIKE :pattern", {"pattern": "%2d%"}
        )
        db.execute("DELETE FROM sessions WHERE category_id = :cat", {"cat": "boids_2d"})
        db.close()

    def test_2d_boids_data_integrity(
        self, backend_config: DBConfig, sample_2d_boids_data: Path
    ):
    ...
    def test_2d_boids_velocity_computation(
        self, backend_config: DBConfig, sample_2d_boids_data: Path
    ):
        ...
        assert abs(vx0 - expected_vx) < 0.01, (
            f"v_x mismatch: expected {expected_vx:.4f}, got {vx0:.4f}"
        )
        assert abs(vy0 - expected_vy) < 0.01, (
            f"v_y mismatch: expected {expected_vy:.4f}, got {vy0:.4f}"
        )
    ...
    def test_2d_boids_episode_metadata(
        self, backend_config: DBConfig, sample_2d_boids_data: Path
    ):
