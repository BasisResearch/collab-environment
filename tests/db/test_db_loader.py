...
class TestDataLoader:
    ...
    def test_load_session(self, backend_config: DBConfig):
        ...
        assert result_row[0] == "Test Session"

        # Cleanup
        db.execute(
            "DELETE FROM sessions WHERE session_id = :sid", {"sid": "test-session-1"}
        )
        db.close()
    ...
    def test_load_episode(self, backend_config: DBConfig):
        ...
        assert result_row is not None
        assert result_row[0] == 10

        # Cleanup
        db.execute(
            "DELETE FROM episodes WHERE episode_id = :eid", {"eid": "test-episode-1"}
        )
        db.execute(
            "DELETE FROM sessions WHERE session_id = :sid", {"sid": "test-session-2"}
        )
        db.close()

    def test_load_boids_simulation(
        self, backend_config: DBConfig, sample_boids_data: Path
    ):
        ...
        assert result[0] == 70, (
            f"Expected 70 observations (5 agents + 2 env), found {result[0]}"
        )

        # Cleanup
        db.execute(
            "DELETE FROM extended_properties WHERE observation_id IN (SELECT observation_id FROM observations WHERE episode_id LIKE :pattern)",
            {"pattern": "%test%"},
        )
        db.execute(
            "DELETE FROM observations WHERE episode_id LIKE :pattern",
            {"pattern": "%test%"},
        )
        db.execute(
            "DELETE FROM episodes WHERE session_id LIKE :pattern",
            {"pattern": f"%{session_name}%"},
        )
        db.execute(
            "DELETE FROM sessions WHERE session_name = :name", {"name": session_name}
        )
        db.close()

    def test_observations_data_integrity(
        self, backend_config: DBConfig, sample_boids_data: Path
    ):
    ...
    def test_extended_properties_loaded(
        self, backend_config: DBConfig, sample_boids_data: Path
    ):
...
class TestGNNRolloutLoader:
    ...
    def rollout_file(self):
        ...
        rollout_path = Path(
            "trained_models/boid_food_strong/rollouts/boid_food_strong_vpluspplus_a_n0_h1_vr0.5_s0_rollout_5.pkl"
        )
    ...
    def test_load_gnn_rollout(self, backend_config: DBConfig, rollout_file: Path):
        ...
        loader = GNNRolloutLoader(
            db, max_episodes=2
        )  # Load only 2 trajectories for speed
        ...
        assert result is not None and result[0] >= 1, (
            "Should have created rollout session"
        )
        ...
        assert result is not None, "Should have episode count result"
        assert result[0] >= 2, f"Should have at least 2 episodes, found {result[0]}"

        # Verify observations were loaded
        result = db.fetch_one("SELECT COUNT(*) FROM observations")
        assert result is not None and result[0] > 0, (
            "Should have loaded some observations"
        )

        # Cleanup
        db.execute(
            "DELETE FROM extended_properties WHERE observation_id IN (SELECT observation_id FROM observations WHERE episode_id LIKE 'rollout-%')"
        )
    ...
    def test_prediction_error_property(
        self, backend_config: DBConfig, rollout_file: Path
    ):
        ...
        count = result[0]
        assert count > 0, (
            f"Should have prediction_error for predicted episode, found {count}"
        )
        ...
        assert min_val >= 0, (
            f"prediction_error should be non-negative, found min={min_val}"
        )
        assert max_val < 480.0, (
            f"prediction_error should be < scene_size, found max={max_val}"
        )
        ...
        assert result is not None and result[0] == 0, (
            "Should NOT have prediction_error for actual episodes"
        )
    ...
    def test_loss_env_agent(self, backend_config: DBConfig, rollout_file: Path):
        ...
        assert env_obs_count > 0, (
            f"Should have env observations in predicted episode, found {env_obs_count}"
        )
        ...
        loss_count = result[0]
        assert loss_count > 0, (
            f"Should have loss property for env agent, found {loss_count}"
        )
        assert loss_count == env_obs_count, (
            f"Should have one loss per env observation, found {loss_count} vs {env_obs_count}"
        )
